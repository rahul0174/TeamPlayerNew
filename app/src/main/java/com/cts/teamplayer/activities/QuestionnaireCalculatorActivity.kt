...
import com.cts.teamplayer.adapters.AnswerAdapterNew
...
class QuestionnaireCalculatorActivity : AppCompatActivity() , View.OnClickListener, ItemClickListner {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.fragment_questionnairecalculator)

        mpref = TeamPlayerSharedPrefrence.getInstance(this@QuestionnaireCalculatorActivity)
        findId()
    }
    private fun findId() {
        showCounterTimer()
        questionAnswerList()
        btn_submit_questionnaire.setOnClickListener(this)
    }
                ...
                if (question_size!! >= i!!) {
                    if(radioButton.equals("1")){
                        ...
                        if(selectedList.size==3){
                            ...
                            Toast.makeText(
                                this@QuestionnaireCalculatorActivity,
                                "Please select max 3 ",
                                Toast.LENGTH_LONG
                            ).show()
                        }*/
    ...
    public fun addAapter(list: ArrayList<AnswersItemNew>?){
        var manager = LinearLayoutManager(this@QuestionnaireCalculatorActivity, LinearLayoutManager.VERTICAL, false)
        recyler_question_answer.layoutManager = manager


        val   answerListAdapter =  AnswerAdapterNew(this@QuestionnaireCalculatorActivity, list, this, main_answer!!,this@QuestionnaireCalculatorActivity)
    ...
    private fun questionAnswerList(){
        if (CheckNetworkConnection.isConnection1(this@QuestionnaireCalculatorActivity, true)) {
            val progress = ProgressDialog(this@QuestionnaireCalculatorActivity)
            progress.setMessage(resources.getString(R.string.please_wait))
            progress.setCancelable(false)
            progress.isIndeterminate = true
            progress.show()
            val apiInterface = ApiClient.getConnection(this@QuestionnaireCalculatorActivity)
                ...
                override fun onResponse(
                    ...
                    if (response.code() >= 200 && response.code() < 210) {
                            ...
                            for (j in 0..questionList!!.size-1) {
                                ...
                                if(response.body()!!.data!!.questions!!.get(j)!!.answerSaved!!.equals(true)){
                                    ...
                                    if(response.body()!!.data!!.questions!!.size-1==i){
                                        ...
                                        val jsonObject = JsonObject()
                                        jsonObject.addProperty("test", question_id)
                                        jsonObject.addProperty("group_id", answer_id)
                        ...
                        try {
                            ...
                            Toast.makeText(this@QuestionnaireCalculatorActivity, message, Toast.LENGTH_LONG).show()
                        } catch (e: JSONException) {
                            e.printStackTrace()
                            Toast.makeText(
                                this@QuestionnaireCalculatorActivity,
    ...
    override fun onClickItem(position: Int, requestcode: Int) {
        ...
        if (requestcode== MyConstants.QUESTION_WITH_ANSWER_CHECKBOX){
            radioButton="0"
        }
    ...
    private fun showCounterTimer(){
        countDownTimer = object : CountDownTimer(90000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                tv_minutes.setText("" + millisUntilFinished / 1000 / 60)
                tv_seond.setText("" + seconds)
                ...
                override fun onResponse(
                    ...
                    if (response.code() >= 200 && response.code() < 210) {
                        try {
                            ...
                            mpref!!.setRoal(role)
                            val i = Intent(this@QuestionnaireCalculatorActivity, MainActivity::class.java).addFlags(
